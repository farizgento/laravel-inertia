...
class ActivityLogController extends Controller
{
    ...
    private function fieldLabel(string $field): string
    {
        return match ($field) {
            ...
            'requester_reviewed_by' => 'Reviewer Area Peminjam',
            'reviewed_at' => 'Di Review Pada',
            'requester_reviewed_at' => 'Tanggal Review Area Peminjam',
            'requester_review_note' => 'Catatan Review Area Peminjam',
            'tanggal_pinjam' => 'Tanggal Pinjam',
            'tanggal_kembali' => 'Tanggal Kembali',
            'pekerjaan' => 'Pekerjaan',
            'kategori' => 'Kategori',
            'is_inter_area' => 'Antar Area',
            'approved_qty' => 'Qty Disetujui',
            'returned_qty' => 'Qty Dikembalikan',
            'qty' => 'Qty Diminta',
            ...
            'user_id' => 'User',
            'area_id' => 'Area',
            'requester_area_id' => 'Area Peminjam',
            'source_peminjaman_id' => 'Sumber Peminjaman',
            'alat_id' => 'Alat',
            'role_id' => 'Role',
    ...
    private function displayFieldValue(string $field, mixed $value): mixed
    {
        ...
        return match ($field) {
            'status' => $value === 'Dipesan' ? 'Disetujui' : $value,
            'reviewed_by', 'requester_reviewed_by', 'user_id' => User::query()->find($value)?->name ?? $value,
            'area_id', 'requester_area_id' => Area::query()->find($value)?->name ?? $value,
            'role_id' => Role::query()->find($value)?->name ?? $value,
            'alat_id' => Alat::query()->find($value)?->nama ?? $value,
            'peminjaman_id', 'source_peminjaman_id' => Peminjaman::query()->find($value)?->pekerjaan ?? "Peminjaman #{$value}",
            'is_inter_area' => $value ? 'Ya' : 'Tidak',
            default => $value,
        };
    }
