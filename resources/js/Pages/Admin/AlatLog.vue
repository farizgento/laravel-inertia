            ...
            <div v-if="isLoading" class="overflow-hidden rounded-2xl border border-slate-200">
                ...
                <div class="divide-y divide-slate-100 bg-white">
                    <div v-for="row in 5" :key="`skeleton-${row}`" class="flex items-center gap-4 px-4 py-4">
                        ...
                        <div class="h-6 w-16 shrink-0 animate-pulse rounded-full bg-slate-100"></div>
                    </div>
                </div>
            </div>
                ...
                <p class="mx-auto mt-1 max-w-md text-sm text-slate-500">
                    {{
                        hasActiveFilters
                            ...
                            : 'Perubahan data alat akan tercatat di sini setelah ada penambahan, update stok, atau penghapusan alat.'
                    }}
                </p>
                <button
            ...
            <div v-else>
                    ...
                    <div class="max-h-[68vh] overflow-auto">
                        <table class="w-full min-w-[980px] text-sm">
                            <caption class="sr-only">
                                Daftar perubahan data alat beserta pergerakan stoknya
                            </caption>
                            <thead class="sticky top-0 z-10 bg-slate-50">
                                <tr class="text-left text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                                    ...
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Aksi</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Alat</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Stok</th>
                            ...
                            <tbody class="divide-y divide-slate-100 bg-white">
                                <template v-for="log in logs" :key="log.id">
                                    <tr class="align-top transition hover:bg-slate-50/70">
                                        ...
                                        <td class="px-4 py-4">
                                            <span
                                                ...
                                                :class="actionBadgeClass(log)"
                                            >
                                                <span class="h-1.5 w-1.5 rounded-full" :class="actionDotClass(log)" aria-hidden="true"></span>
                                                {{ log.action_label }}
                                            </span>
                                        </td>
                                        <td class="max-w-[20rem] px-4 py-4">
                                            <p class="font-semibold text-slate-900">{{ log.alat_name }}</p>
                                            <p class="mt-0.5 font-mono text-xs text-slate-500">{{ log.alat_code }}</p>
                                            <p
                                                v-if="alatTypeLabel(log)"
                                                class="mt-1 line-clamp-1 text-xs text-slate-500"
                                                :title="alatTypeLabel(log)"
                                            >
                                                {{ alatTypeLabel(log) }}
                                            </p>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div v-if="log.stock_delta !== null" class="flex flex-wrap items-center gap-1.5">
                                                <span class="tabular-nums text-slate-500">{{ log.total_aset_before }}</span>
                                                ...
                                                <span class="font-semibold tabular-nums text-slate-900">{{ log.total_aset_after }}</span>
                                                <span
                                                ...
                                                >
                                                    <svg
                                                        v-if="log.stock_delta !== 0"
                                                        class="h-3 w-3"
                                                        :class="log.stock_delta < 0 ? 'rotate-180' : ''"
                                            ...
                                            <div v-else class="text-slate-600">
                                                <span v-if="currentTotal(log) !== '-'" class="tabular-nums">{{ currentTotal(log) }}</span>
                                                <span v-else class="text-xs italic text-slate-400">Tidak berubah</span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4 text-right">
                                            ...
                                            >
                                                {{ isExpanded(log.id) ? 'Tutup' : `Detail (${log.changes.length})` }}
                                                <svg
                                    ...
                                    <tr v-if="isExpanded(log.id)" :key="`${log.id}-detail`" class="bg-slate-50/80">
                                        <td :id="`alat-log-detail-${log.id}`" colspan="6" class="px-4 py-4">
                                            <div class="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-2">
                                                <ActivityChangeList title="Sebelum" tone="rose" :changes="beforeChanges(log)" />
                                                <ActivityChangeList title="Sesudah" tone="emerald" :changes="afterChanges(log)" />
                                            </div>
                                        </td>
                                    </tr>
                                </template>
                ...
                <ul class="space-y-3 md:hidden">
                    ...
                    >
                        ...
                        <div v-if="log.stock_delta !== null" class="mt-3 flex flex-wrap items-center gap-1.5 rounded-xl bg-slate-50 px-3 py-2 text-sm">
                            <span class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Stok</span>
                            <span class="tabular-nums text-slate-500">{{ log.total_aset_before }}</span>
                            <svg class="h-3.5 w-3.5 shrink-0 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                <path d="M5 12h14M13 6l6 6-6 6" />
                            </svg>
                        ...
                        <dl class="mt-3 space-y-1.5 text-sm">
                            ...
                            <div class="flex gap-2">
                                ...
                                <dd class="min-w-0 text-slate-600">
                                    <span class="font-semibold text-slate-900">{{ log.actor_name }}</span>
                            ...
                            <div v-if="alatTypeLabel(log)" class="flex gap-2">
                                <dt class="w-24 shrink-0 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Jenis</dt>
                                <dd class="min-w-0 text-slate-600">{{ alatTypeLabel(log) }}</dd>
                            </div>
                        </dl>
                        ...
                        <button
                            ...
                            @click="toggleExpanded(log.id)"
                        >
                            {{ isExpanded(log.id) ? 'Tutup detail perubahan' : `Lihat ${log.changes.length} detail perubahan` }}
                            <svg
                                class="h-3.5 w-3.5 transition-transform"
                                :class="isExpanded(log.id) ? 'rotate-180' : ''"
                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"
                        ...
                        <div
                            ...
                            class="mt-3 space-y-4 rounded-xl border border-slate-200 bg-slate-50/70 p-3"
                        >
                            <ActivityChangeList title="Sebelum" tone="rose" :changes="beforeChanges(log)" />
                            <ActivityChangeList title="Sesudah" tone="emerald" :changes="afterChanges(log)" />
                        </div>
...
import ActivityChangeList from '../../Components/ActivityChangeList.vue';
...
const hasChanges = (log) => Boolean(log.changes?.length);
...
// Backend mengirim perubahan sebagai satu daftar {field,label,before,after}. Dipecah
// menjadi dua sisi supaya bisa memakai ActivityChangeList yang sama dengan Log Activity.
const beforeChanges = (log) =>
...
const alatTypeLabel = (log) => {
    const parts = [log.jenis_alat, log.klasifikasi_alat].filter(
        (part) => part && part !== '-'
    );
...
// Dipakai saat stok tidak berubah: tampilkan angka yang tersedia apa adanya.
const currentTotal = (log) => {
    const after = log.total_aset_after;
    if (after !== null && after !== undefined && after !== '-') {
        return after;
    }

    const before = log.total_aset_before;
    return before === null || before === undefined ? '-' : before;
};

// Warna diturunkan dari aksi mentah dan selisih stok, bukan dari teks label,
// supaya tetap benar bila kata pada label diubah di backend.
const actionTone = (log) => {
    ...
    if (log.action === 'update' && Number(log.stock_delta) > 0) {
        return 'emerald';
    }
    if (log.action === 'update' && Number(log.stock_delta) < 0) {
        return 'amber';
...
// Titik warna hanya penegas; label teks aksi tetap menjadi pembeda utamanya.
const actionDotClass = (log) => {
    switch (actionTone(log)) {
        case 'emerald':
