    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/50">
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end">
            <div class="min-w-0 flex-1">
                <label for="activity-search" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                    Cari Aktivitas
                </label>
                <div class="relative">
            ...
            <div class="w-full lg:w-48">
                <label for="activity-action" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">
                    Aksi
                </label>
                <select
                    id="activity-action"
        ...
        <div class="mt-5" aria-live="polite" :aria-busy="isLoading">
            ...
            <div v-if="isLoading" class="overflow-hidden rounded-2xl border border-slate-200">
                ...
                <div class="divide-y divide-slate-100 bg-white">
                    <div v-for="row in 5" :key="`skeleton-${row}`" class="flex items-center gap-4 px-4 py-4">
                        ...
                        <div class="h-9 w-40 shrink-0 animate-pulse rounded-lg bg-slate-100"></div>
            ...
            <div v-else-if="!logs.length" class="rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 px-6 py-12 text-center">
                ...
                <p class="mt-3 text-sm font-semibold text-slate-700">
                    {{ hasActiveFilters ? 'Tidak ada aktivitas yang cocok' : 'Belum ada activity log' }}
                </p>
                <p class="mx-auto mt-1 max-w-md text-sm text-slate-500">
                    {{
                        hasActiveFilters
                            ? 'Coba ubah kata kunci atau pilih aksi lain untuk memperluas hasil pencarian.'
                            : 'Aktivitas pengguna akan muncul di sini setelah ada perubahan data pada sistem.'
                    }}
                </p>
                <button
                    ...
                    <div class="max-h-[68vh] overflow-auto">
                        <table class="w-full min-w-[900px] text-sm">
                            <caption class="sr-only">
                                Daftar aktivitas pengguna beserta perubahan data yang dilakukan
                            </caption>
                            <thead class="sticky top-0 z-10 bg-slate-50">
                                <tr class="text-left text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Waktu</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">User</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Aksi</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Data</th>
                                    <th scope="col" class="border-b border-slate-200 px-4 py-3">Deskripsi</th>
                                ...
                                <template v-for="log in logs" :key="log.id">
                                    <tr class="align-top transition hover:bg-slate-50/70">
                                        ...
                                        <td class="px-4 py-4">
                                            ...
                                            >
                                                <span class="h-1.5 w-1.5 rounded-full" :class="actionDotClass(log.action)" aria-hidden="true"></span>
                                                {{ log.action_label }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-4">
                                            <p class="font-semibold text-slate-900">{{ log.subject_type }}</p>
                                            <p class="mt-0.5 line-clamp-2 text-xs text-slate-500" :title="log.subject_label">
                                                {{ log.subject_label }}
                                            </p>
                                        </td>
                                        <td class="max-w-[22rem] px-4 py-4">
                                            <p class="line-clamp-2 text-slate-600" :title="log.description">{{ log.description }}</p>
                                        </td>
                                        <td class="px-4 py-4 text-right">
                                            <button
                                                ...
                                                :aria-controls="`activity-detail-${log.id}`"
                                    ...
                                    <tr v-if="isExpanded(log.id)" :key="`${log.id}-detail`" class="bg-slate-50/80">
                                        <td :id="`activity-detail-${log.id}`" colspan="6" class="px-4 py-4">
                                            <div class="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-2">
                                                <ActivityChangeList title="Sebelum" tone="rose" :changes="log.before_changes" />
                                                <ActivityChangeList title="Sesudah" tone="emerald" :changes="log.after_changes" />
                                            </div>
                                        </td>
                                    </tr>
                                </template>
                    ...
                    >
                        ...
                        <dl class="mt-3 space-y-1.5 text-sm">
                            <div class="flex gap-2">
                                <dt class="w-20 shrink-0 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Waktu</dt>
                            ...
                            <div class="flex gap-2">
                                <dt class="w-20 shrink-0 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Data</dt>
                                <dd class="min-w-0 text-slate-600">
                                    <span class="font-semibold text-slate-900">{{ log.subject_type }}</span>
                                    <span class="block text-xs text-slate-500">{{ log.subject_label }}</span>
                                </dd>
                            </div>
                            <div class="flex gap-2">
                                <dt class="w-20 shrink-0 text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Detail</dt>
                                <dd class="min-w-0 text-slate-600">{{ log.description }}</dd>
                            </div>
                        </dl>
                        ...
                        <button
                            ...
                            type="button"
                            :aria-expanded="isExpanded(log.id)"
                            :aria-controls="`activity-detail-card-${log.id}`"
                            @click="toggleExpanded(log.id)"
                        >
                        ...
                        <div
                            ...
                            :id="`activity-detail-card-${log.id}`"
        ...
        <div class="mt-4 flex flex-col gap-3 border-t border-slate-200 pt-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
                <p class="text-sm text-slate-500">
                    <template v-if="pagination.total">
                        ...
                        dari <span class="font-semibold text-slate-700">{{ pagination.total }}</span> aktivitas
                    </template>
                    <template v-else>Total {{ pagination.total }} aktivitas</template>
                </p>
                <div class="flex items-center gap-2">
                    <label for="activity-per-page" class="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">
                        Baris
                    </label>
                    <select
            ...
            <nav class="flex items-center gap-1" aria-label="Navigasi halaman log activity">
                <button
...
const hasChanges = (log) => Boolean(log.before_changes?.length || log.after_changes?.length);
