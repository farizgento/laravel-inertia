<template>
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-500">Pantau laporan kerusakan alat per area</p>
    </div>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/50">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <h2 class="text-lg font-semibold text-slate-900">Data Kerusakan</h2>
        ...
        >
            ...
            <div class="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900">Tambah Laporan Kerusakan</h3>
                        <p class="mt-1 text-sm text-slate-500">Isi detail kerusakan alat yang ditemukan.</p>
                    </div>
                    <button
    ...
    <teleport to="body">
        ...
        >
            ...
            <div class="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900">Detail Kerusakan</h3>
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <div v-if="detailItem?.url" class="mt-4 overflow-hidden rounded-2xl border border-slate-200">
                            <img :src="detailItem.url" alt="Foto kerusakan" class="h-56 w-full object-cover" />
                        </div>
                    </div>
                </div>
            </div>
    ...
    layout: (h, page) =>
        h(
            AppLayout,
            {
                ...
                subtitle: 'Pantau laporan kerusakan alat per area',
                activeMenu: 'laporan-kerusakan',
            },
            () => page
        ),
});
...
const loadReports = async () => {
    ...
    loadError.value = '';
    try {
        const response = await axios.get('/api/laporan-kerusakan', {
            params: buildParams(),
        });
        ...
        reports.value = [];
        loadError.value = 'Gagal memuat laporan kerusakan.';
...
const submitForm = async () => {
    ...
    try {
        ...
        await axios.post('/api/laporan-kerusakan', payload, {
            headers: {
        ...
        closeForm();
        showAlert('success', 'Laporan kerusakan berhasil dikirim.');
    } catch (error) {
...
const submitStatus = async () => {
    ...
    statusError.value = '';
    try {
        await axios.put(`/api/laporan-kerusakan/${selectedStatusItem.value.id}`, {
        ...
        await refreshSidebarNotificationCounts();
        closeStatusModal();
        showAlert('success', 'Status laporan kerusakan berhasil diperbarui.');
    } catch (error) {
        statusError.value =
